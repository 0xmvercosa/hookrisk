...
- **Price impacting behavior**
  - Not measured: custom-accounting requires hookrisk; hookrisk ran but did not analyse the target (TakeProfitsHook uses a hook ABI hookrisk cannot analyse: TakeProfitsHook (src/TakeProfitsHook.sol#17-391) looks like a Uniswap v4 hook but its hook ABI predates the shipped v4 interface (declares the 2023 …); no detector for unbounded-dynamic-fee yet.
